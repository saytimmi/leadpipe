...
export async function GET(req: NextRequest) {
  ...
  const [pvRes, svRes, feRes] = await Promise.all([
    supabase.from("lp_page_views").select("session_id, referrer, utm_source").gte("created_at", since),
    supabase.from("lp_section_views").select("session_id, section, time_spent_ms").gte("created_at", since),
    supabase.from("lp_form_events").select("session_id, event, step_name").gte("created_at", since),
  ]);
  ...
  lines.push("", "🔗 Подробнее: https://leadpipe.dev/stats");
